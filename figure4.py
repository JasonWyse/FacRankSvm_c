...
if len(data) > 0 and len(methodlist) > 0:
	...
	p.write(solverlist)
	p.close()
	#copy the file fig4.m to tmp.m except the first two line,the first two lines are inputlist(ie. data) and solverlist(ie. methodlist) we configurate in this file,we don't use the inputlist and solverlist in fig4.m
