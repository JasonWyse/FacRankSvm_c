...
cmd = "make -C tools >/dev/null 2>/dev/null;mkdir model log 2>/dev/null"
system(cmd)

#remove those method/data you are not interested in its result
methodlist = ['random-forest','gbdt']
data = ['MQ2007','MQ2008','YAHOO_SET2']
...
for d in data:
	...
	for method in methodlist:
		dp = log_path + d + '.' + method+ '.manytrees.log'
		try:
			...
			if method == 'random-forest':
				cmd = "%s -f %s -F -z -p %s -k %s -t %s %s %s ./tmp_file >> %s 2>/dev/null"%(tree_exe,num_feature[d],num_processors, num_sampled_feature[d], tree_num_more[method],traindata,testdata,dp)
			elif method == 'gbdt':
				model = model_path + d + '.' + method + '.' + 'manytrees.model'
				cmd = "mpirun -np %s %s %s %s %s 4 1000 0.1 -m >%s 2> %s"%(8,gbrt_exe,traindata,num_instance[d],num_feature[d]+1,model,dp)
			...
			system('echo \'%s\' >> %s'%(cmd, dp))
			system(cmd)
			cmd = "tools/eval ./tmp_file %s >> %s;rm -f tmp_file"%(testdata, dp)
			system('echo \'%s\' >> %s'%(cmd, dp))
			system(cmd)
			tmp_data = open(dp,'r').readlines()
		for l in tmp_data:
